    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 rounded-2xl flex items-center justify-center shadow-inner">
            <Shield size={24} strokeWidth={2.5} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tighter">
               {t('admin_mgmt_title')}
            </h1>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 capitalize">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden min-h-[400px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center p-20 opacity-50">
            <div className="w-8 h-8 border-4 border-indigo-100 dark:border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin mb-4" />
            <p className="text-xs font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{t('status_loading')}</p>
          </div>
        ) : admins.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-20 text-center">
            <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800/40 rounded-full flex items-center justify-center mb-6">
              <Users size={32} className="text-slate-300 dark:text-slate-600" />
            </div>
            <p className="text-base font-bold text-slate-800 dark:text-slate-200 mb-2">{t('admin_mgmt_no_data')}</p>
            <button onClick={() => openModal()} className="text-indigo-500 dark:text-indigo-400 font-bold hover:underline">
               {t('admin_mgmt_add_btn')}
            </button>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800/60">
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('admin_mgmt_table_user')}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('admin_mgmt_table_role')}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('admin_mgmt_table_added')}</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-right">{t('admin_mgmt_table_actions')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800/40">
                {admins.map((admin) => (
                  <tr key={admin.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-500 dark:text-slate-400 text-xs uppercase">
                           {admin.username.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-2">
                             {admin.username}
                             {admin.isPrimary && <span className="px-2 py-0.5 rounded-md bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[9px] uppercase tracking-widest border border-amber-200 dark:border-amber-500/20">{t('admin_mgmt_badge_primary')}</span>}
                             {admin.id === currentAdmin.id && <span className="px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[9px] uppercase tracking-widest border border-indigo-100 dark:border-indigo-500/20">{t('admin_mgmt_badge_you')}</span>}
                          </p>
                    ...
                    <td className="px-6 py-4">
                      {admin.isPrimary ? (
                         <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-500/20">
                           <CheckCircle size={12} /> {t('admin_mgmt_role_primary')}
                         </span>
                      ) : isFullPrivileges(admin) ? (
                         <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest border border-blue-100 dark:border-blue-500/20">
                           <Shield size={12} /> {t('admin_mgmt_role_full')}
                         </span>
                      ) : (
                         <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        {currentAdmin?.isPrimary && !admin.isPrimary && (
                            <button 
                              ...
                              className="p-2 text-slate-400 dark:text-slate-500 hover:text-amber-500 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-xl transition-all"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
             <div>
               ...
               <input
                 ...
                 className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:focus:bg-slate-950 transition-all"
             ...
             <div>
               ...
               <div className="relative">
                 <input
                   ...
                   className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-800 dark:text-slate-200 pr-12 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:focus:bg-slate-950 transition-all"
                   placeholder="email@example.com"
                 />
                 <div className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-300 dark:text-slate-500">
                   <Mail size={16} />
                 </div>
               </div>
             </div>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800/60">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
               {[
                 ...
                 <label 
                   ...
                   className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                     formData[perm.key] ? 'bg-indigo-50/50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/30' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50'
                   }`}
                 >
                   <span className={`text-xs font-bold ${formData[perm.key] ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}>
                     {perm.label}
                   </span>
                   <div 
                     className={`w-10 h-6 rounded-full p-1 transition-colors duration-200 ease-in-out ${
                       formData[perm.key] ? 'bg-indigo-500' : 'bg-slate-200 dark:bg-slate-700'
                     }`}
                   >
                     <div 
              ...
              <div className="mt-4">
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Message Permissions</label>
                <select 
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={closeModal}
              className="flex-1 px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
            >
              {t('btn_cancel')}
            </button>
            <button
        ...
        <form onSubmit={handleTransferPrimary} className="space-y-4">
          <div className="p-4 bg-amber-50 dark:bg-amber-500/10 rounded-xl border border-amber-200 dark:border-amber-500/20">
            <p className="text-sm font-medium text-amber-800 dark:text-amber-400">
              {t('admin_mgmt_transfer_warn')} <span className="font-bold text-amber-900 dark:text-amber-200">{transferTarget?.username}</span>. 
              {t('admin_mgmt_transfer_warn_2')}
            </p>
          </div>
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={() => setIsTransferModalOpen(false)} className="flex-1 px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700">{t('btn_cancel')}</button>
