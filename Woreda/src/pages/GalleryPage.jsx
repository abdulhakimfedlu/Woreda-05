...
const GalleryCard = ({ item, onClick }) => {
  ...
  return (
    <div
      ...
      className="group relative cursor-pointer overflow-hidden rounded-3xl aspect-[4/3] bg-black/10 dark:bg-white/5 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand/20"
    >
      <img src={item.url} alt={title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
        <h3 className="text-base font-black text-white leading-snug line-clamp-2">{title}</h3>
...
const Lightbox = ({ item, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-6" onClick={onClose}>
      <div className="relative bg-white dark:bg-[#111827] rounded-3xl max-w-4xl w-full overflow-hidden shadow-2xl dark:shadow-brand/10 animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-black/60 transition-colors shadow-lg">
          <X className="w-5 h-5 text-white" />
        </button>

        <div className="flex flex-col md:flex-row">
          <div className="md:w-2/3 h-[300px] md:h-[500px] bg-slate-900 dark:bg-black border-r border-black/10 dark:border-white/10">
            <img src={item.url} alt={title} className="w-full h-full object-contain" />
          </div>
          <div className="md:w-1/3 p-8 flex flex-col">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              <span className="flex items-center gap-1.5 px-3 py-1 bg-black/5 dark:bg-white/5 rounded-full text-[9px] font-black text-black/40 dark:text-white/40 uppercase tracking-widest">
                <Calendar className="w-2.5 h-2.5" /> {item.date || (item.createdAt ? new Date(item.createdAt).toLocaleDateString() : 'N/A')}
              </span>
            </div>
            <h2 className="text-2xl font-black text-black dark:text-white mb-3 tracking-tighter leading-snug">{title}</h2>
            <p className="text-sm text-black/50 dark:text-white/40 font-medium leading-relaxed flex-1">{description}</p>

            <div className="mt-8 pt-6 border-t border-black/8 dark:border-white/8 flex items-center justify-between">
              <a href={item.url} target="_blank" rel="noreferrer" className="text-[10px] font-black text-brand uppercase tracking-widest hover:underline">
...
const GalleryPage = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/gallery')
      .then(res => res.json())
      .then(data => { setGalleryItems(Array.isArray(data) ? data : []); setLoading(false); })
      .catch(() => { setGalleryItems([]); setLoading(false); });
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="flex items-center justify-center gap-6 mb-12">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black text-black/40 dark:text-white/40 uppercase tracking-widest">
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {galleryItems.filter(i => i != null).map(item => (
              <GalleryCard key={item.id} item={item} onClick={setSelectedItem} />
            ))}
          </div>
        ) : (
          <div className="py-24 text-center">
            <Camera className="w-12 h-12 text-black/10 dark:text-white/10 mx-auto mb-4" />
            <p className="text-xl font-black text-black/20 dark:text-white/20 uppercase tracking-widest">
