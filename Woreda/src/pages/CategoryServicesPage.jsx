    ...
    fetch('http://localhost:5000/api/services')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          const filtered = data.filter(s => s.category === decodedCategoryName);
          setServicesData(filtered);
        } else {
          console.error("Received malformed services data:", data);
          setServicesData([]);
        }
        setLoading(false);
      })
      ...
      <div className="max-w-[1000px] mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="mb-10 pt-8">
          ...
          <p className="text-xl text-slate-400 font-bold max-w-2xl">
            Browse and select a specific service to view detailed requirements and procedures.
          </p>
        </div>
        ...
        <div className="relative group mb-12">
          ...
          <input
            type="text"
            placeholder={`Search ${decodedCategoryName} services...`}
            value={searchTerm}
        ...
        {loading ? (
          <div className="py-20 text-center">
             <p className="text-xl font-black text-slate-300 uppercase tracking-widest animate-pulse">Loading Services...</p>
          </div>
            ...
            {filteredServices.map(service => (
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800 tracking-tight group-hover:text-brand transition-colors">{service.title}</h3>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{service.department}</p>
                  </div>
                </div>
          ...
          <div className="py-20 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
             ...
             <p className="text-xl font-black text-slate-400 uppercase tracking-widest">No matching services found</p>
          </div>
        )}
